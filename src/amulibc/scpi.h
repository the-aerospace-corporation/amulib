/**
 * @file scpi.h
 * @brief SCPI Command Interface for AMU Library
 * 
 * This file defines the SCPI (Standard Commands for Programmable Instruments)
 * command interface for the AMU (Aerospace Measurement Unit) library.
 *
 * @author CJM28241
 * @date 5/28/2018
 */ 
...
// A chunk shares the parser buffer with its own command text -
// "SYSTem:FIRMware:DATA " + "#41996" + terminator, rounded well up
#define AMU_OTA_CHUNK_SCPI (AMULIBC_SCPI_INPUT_BUFFER_LENGTH - 64)
...
extern "C" {
	...
	SCPI_CMD_RW_PROTOTYPE(quad_photo_sensor_t)
    ...
    SCPI_CMD_EXEC_QRY_PROTOTYPE(amu_meas_t)
	
	#define SCPI_CMD_WRITE_PROTOTYPE(TYPE)	scpi_result_t scpi_cmd_write_##TYPE(scpi_t *context);
	SCPI_CMD_WRITE_PROTOTYPE(ivsweep_config_t)
	SCPI_CMD_WRITE_PROTOTYPE(ivsweep_meta_t)
	...
	// Extended commands. Every one of these carries CMD_SYSTEM_EXTENDED, so a single
	// command byte reaches the device over TWI; the handlers differ only in which
	// CMD_EXT_t they stage into the transfer register.
	...
	scpi_result_t scpi_cmd_firmware_abort(scpi_t *context);
        ...
        SCPI_COMMAND("SYSTem:SCPI:LIST?",				scpi_cmd_execute,					CMD_USB_SYSTEM_LIST_SCPI_COMMANDS	)	\
                                                                                                                                    \
        SCPI_COMMAND("SYSTem:BLE[?]",					scpi_cmd_ble_state,					CMD_SYSTEM_EXTENDED					)	\
        SCPI_COMMAND("SYSTem:BLE:PASSkey[?]",			scpi_cmd_ble_passkey,				CMD_SYSTEM_EXTENDED					)	\
        SCPI_COMMAND("SYSTem:BOOTloader",				scpi_cmd_execute,					CMD_USB_SYSTEM_ENTER_BOOTLOADER		)	\
        SCPI_COMMAND("SYSTem:REBoot",					scpi_cmd_execute,					CMD_SYSTEM_RESET					)	\
        SCPI_COMMAND("SYSTem:DEBug#?",					scpi_cmd_rw_float,					CMD_USB_SYSTEM_DEBUG				)	\
        SCPI_COMMAND("SYSTem:FIRMware?",				scpi_cmd_query_str,				    CMD_SYSTEM_FIRMWARE					)	\
        SCPI_COMMAND("SYSTem:FIRMware:BEGin?",			scpi_cmd_firmware_begin,			CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:FIRMware:DATA?",			scpi_cmd_firmware_data,				CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:FIRMware:END?",			scpi_cmd_firmware_end,				CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:FIRMware:STATe?",			scpi_cmd_firmware_state,			CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:FIRMware:COMMit[?]",		scpi_cmd_firmware_commit,			CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:FIRMware:ABORt",			scpi_cmd_firmware_abort,			CMD_SYSTEM_EXTENDED 					)	\
        SCPI_COMMAND("SYSTem:HARDware?",				scpi_cmd_rw_uint8_t,				AMU_REG_SYSTEM_HARDWARE_REVISION	)	\
        SCPI_COMMAND("SYSTem:AMULIB?",					scpi_cmd_query_str,				    CMD_SYSTEM_AMULIB					)	\
        SCPI_COMMAND("SYSTem:LED:PAT",					scpi_cmd_led,						CMD_SYSTEM_LED						)	\
        SCPI_COMMAND("SYSTem:LED:COLOR[?]",				scpi_cmd_rw_amu_pid_t,				CMD_SYSTEM_LED_COLOR				)	\
        SCPI_COMMAND("SYSTem:SERial?",					scpi_cmd_query_str,				    CMD_SYSTEM_SERIAL_NUM				)	\
        ...
        SCPI_COMMAND("SYSTem:TWI:SCAN?",				scpi_cmd_twi_scan,					CMD_USB_SYSTEM_TWI_SCAN				)	\
        ...
        SCPI_COMMAND("SYSTem:XMEGA:SIGnature?",			scpi_cmd_query_str,				    CMD_SYSTEM_XMEGA_USER_SIGNATURES	)	\
                                                                                                                                    \
        ...
        SCPI_COMMAND("DUT:MANufacturer[?]",				scpi_cmd_query_str,				    CMD_DUT_MANUFACTURER				)	\
        SCPI_COMMAND("DUT:MODel[?]",					scpi_cmd_query_str,				    CMD_DUT_MODEL						)	\
        SCPI_COMMAND("DUT:TECHnology[?]",				scpi_cmd_query_str,				    CMD_DUT_TECHNOLOGY					)	\
        SCPI_COMMAND("DUT:SERialnumber[?]",				scpi_cmd_query_str,				    CMD_DUT_SERIAL_NUMBER				)	\
        SCPI_COMMAND("DUT:ENERGY[?]",					scpi_cmd_rw_float,					CMD_DUT_ENERGY						)	\
        SCPI_COMMAND("DUT:DOSE[?]",						scpi_cmd_rw_float,					CMD_DUT_DOSE						)	\
        SCPI_COMMAND("DUT:NOTEs[?]",					scpi_cmd_query_str,				    CMD_DUT_NOTES						)	\
                                                                                                                                    \
        ...
        SCPI_COMMAND("DUT:TSENSor:FIT[?]",				scpi_cmd_rw_amu_coeff_t,			CMD_DUT_TSENSOR_FIT					)	\
                                                                                                                                    \
        SCPI_COMMAND("MEASure:ADC:ACTive[:RAW]?",		scpi_cmd_measure_active_ch,		    CMD_EXEC_MEAS_ACTIVE_CHANNELS		)	\
        SCPI_COMMAND("MEASure:INTERNALvoltages?",		scpi_cmd_rw_amu_int_volt_t,			CMD_EXEC_MEAS_INTERNAL_VOLTAGES		)	\
        SCPI_COMMAND("MEASure:SUNSensor?",				scpi_cmd_rw_quad_photo_sensor_t,	CMD_EXEC_MEAS_SUN_SENSOR			)	\
        ...
        SCPI_COMMAND("ADC:STATus?",						scpi_cmd_rw_uint32_t,				AMU_REG_SYSTEM_STATUS_HRADC			)	\
                                                                                                                                    \
        SCPI_COMMAND("SUNSensor?",						scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_SUNSENSOR			)	\
                                                                                                                                    \
        SCPI_COMMAND("SWEEP:TIMEstamp?",				scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_TIMESTAMP			)	\
        SCPI_COMMAND("SWEEP:VOLTage?",					scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_VOLTAGE			)	\
        SCPI_COMMAND("SWEEP:CURRent?",					scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_CURRENT			)	\
        SCPI_COMMAND("SWEEP:YAW?",						scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_SS_YAW				)	\
        SCPI_COMMAND("SWEEP:PITCH?",					scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_SS_PITCH			)	\
                                                                                                                                    \
        SCPI_COMMAND("SWEEP:VOLTage",					scpi_cmd_write_sweep_ptr,			AMU_REG_DATA_PTR_VOLTAGE			)	\
        SCPI_COMMAND("SWEEP:CURRent",					scpi_cmd_write_sweep_ptr,			AMU_REG_DATA_PTR_CURRENT			)	\
        SCPI_COMMAND("SWEEP:YAW",						scpi_cmd_write_sweep_ptr,			AMU_REG_DATA_PTR_SS_YAW				)	\
        SCPI_COMMAND("SWEEP:PITCH",						scpi_cmd_write_sweep_ptr,			AMU_REG_DATA_PTR_SS_PITCH			)	\
                                                                                                                                    \
        ...
        SCPI_COMMAND("HEATer:PID:SAVE",					scpi_cmd_execute,					CMD_EXEC_HEATER_PID_SAVE			)	\
                                                                                                                                    \
        SCPI_COMMAND("SWEEP:META?",						scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_SWEEP_META			)	\
        SCPI_COMMAND("SWEEP:META",						scpi_cmd_write_ivsweep_meta_t,		AMU_REG_DATA_PTR_SWEEP_META			)	\
                                                                                                                                    \
        SCPI_COMMAND("SWEEP:CONFig?",					scpi_cmd_read_ptr,					AMU_REG_DATA_PTR_SWEEP_CONFIG		)	\
        SCPI_COMMAND("SWEEP:CONFig",					scpi_cmd_write_ivsweep_config_t,	AMU_REG_DATA_PTR_SWEEP_CONFIG		)	\
        ...
        SCPI_COMMAND("SWEEP:TRIGger:ISC?",				scpi_cmd_exec_qry_amu_meas_t,	    CMD_SWEEP_TRIG_ISC					)	\
        SCPI_COMMAND("SWEEP:TRIGger:VOC?",				scpi_cmd_exec_qry_amu_meas_t,		CMD_SWEEP_TRIG_VOC					)	\
    ...
    #define __AMU_EXTENDED_CMD_LIST__																								\
        ...
        SCPI_COMMAND("ADC:VOLTage:PGA:SAVE",			scpi_cmd_execute,					CMD_USB_ADC_VOLTAGE_PGA_SAVE		)	\
                                                                                                                                    \
        ...
        SCPI_COMMAND("ADC:CURRent:PGA:SAVE",			scpi_cmd_execute,					CMD_USB_ADC_CURRENT_PGA_SAVE		)	\
                                                                                                                                    \
        ...
        SCPI_COMMAND("SWEEP:CONFig:AREA[?]",			scpi_cmd_rw_float,					AMU_REG_SWEEP_CONFIG_AREA			)	\
                                                                                                                                    \
        SCPI_COMMAND("SUNSensor:TL?",					scpi_cmd_measure_ss_channel,		CMD_MEAS_CH_SS_TL					)	\
        SCPI_COMMAND("SUNSensor:BL?",					scpi_cmd_measure_ss_channel,		CMD_MEAS_CH_SS_BL					)	\
        SCPI_COMMAND("SUNSensor:BR?",					scpi_cmd_measure_ss_channel,		CMD_MEAS_CH_SS_BR					)	\
        SCPI_COMMAND("SUNSensor:TR?",					scpi_cmd_measure_ss_channel,		CMD_MEAS_CH_SS_TR					)	\
        ...
        SCPI_COMMAND("SUNSensor:THRESHold[?]",			scpi_cmd_rw_float,					CMD_AUX_SUNSENSOR_THRESHOLD			)	\
                                                                                                                                    \
        SCPI_COMMAND("MEASure:ADC:CH#[:RAW]?",			scpi_cmd_measure_channel,			CMD_EXEC_MEAS_CHANNEL				)	\
        SCPI_COMMAND("MEASure:ADC:VOLTage[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_VOLTAGE					)	\
        SCPI_COMMAND("MEASure:ADC:CURRent[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_CURRENT					)	\
        SCPI_COMMAND("MEASure:ADC:TSENSor[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_TSENSOR					)	\
        SCPI_COMMAND("MEASure:ADC:TSENSOR0[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_TSENSOR_0				)	\
        SCPI_COMMAND("MEASure:ADC:TSENSOR1[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_TSENSOR_1				)	\
        SCPI_COMMAND("MEASure:ADC:TSENSOR2[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_TSENSOR_2				)	\
        SCPI_COMMAND("MEASure:ADC:BIAS[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_BIAS					)	\
        SCPI_COMMAND("MEASure:ADC:OFFset[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_OFFSET					)	\
        SCPI_COMMAND("MEASure:ADC:TEMP[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_TEMP					)	\
        SCPI_COMMAND("MEASure:ADC:AVDD[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_AVDD					)	\
        SCPI_COMMAND("MEASure:ADC:IOVDD[:RAW]?",		scpi_cmd_measure_channel,			CMD_MEAS_CH_IOVDD					)	\
        SCPI_COMMAND("MEASure:ADC:ALDO[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_ALDO					)	\
        SCPI_COMMAND("MEASure:ADC:DLDO[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_DLDO					)	\
        SCPI_COMMAND("MEASure:ADC:TSENSORS[:RAW]?",		scpi_cmd_measure_tsensors,			CMD_EXEC_MEAS_TSENSORS				)	\
        SCPI_COMMAND("MEASure:ADC:SSTL[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_SS_TL					)	\
        SCPI_COMMAND("MEASure:ADC:SSBL[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_SS_BL					)	\
        SCPI_COMMAND("MEASure:ADC:SSBR[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_SS_BR					)	\
        SCPI_COMMAND("MEASure:ADC:SSTR[:RAW]?",			scpi_cmd_measure_channel,			CMD_MEAS_CH_SS_TR					)	\
                                                                                                                                    \
